...
module.exports = function ({ Meeting, protect, usingMongo }) {
    ...
    router.put('/:meetingId/evaluate', protect, async (req, res) => {
        try {
            if (!usingMongo()) return res.status(400).json({ message: 'Database required' });
            ...
            const existingIdx = rubric.evaluations.findIndex(
                e => e.participantId.toString() === participantId
            );

            if (existingIdx >= 0) {
                rubric.evaluations[existingIdx].scores = scores;
                rubric.evaluations[existingIdx].participantName = participantName || '';
            } else {
                rubric.evaluations.push({ participantId, participantName: participantName || '', scores });
            }
    ...
    router.get('/:meetingId/report', protect, async (req, res) => {
        try {
            if (!usingMongo()) return res.status(400).json({ message: 'Database required' });
            ...
            const report = {
                ...
                evaluations: rubric.evaluations.map(e => ({
                    participant: e.participantId?.name || e.participantName || 'Unknown',
                    email: e.participantId?.email || '',
                    scores: e.scores.map(s => ({
                        criterion: rubric.criteria[s.criterionIndex]?.name || `Criterion ${s.criterionIndex}`,
                        maxScore: rubric.criteria[s.criterionIndex]?.maxScore || 10,
                        score: s.score,
                        comment: s.comment,
                        transcriptTimestamp: s.transcriptTimestamp,
                    })),
                    totalScore: e.scores.reduce((sum, s) => sum + s.score, 0),
                    maxPossible: rubric.criteria.reduce((sum, c) => sum + c.maxScore, 0),
                })),
            };
