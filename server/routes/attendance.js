...
module.exports = function ({ Meeting, protect, usingMongo, JWT_SECRET, PORT }) {

    router.post('/:meetingId/generate-qr', protect, async (req, res) => {
        try {
    ...
    router.get('/:meetingId/mark', async (req, res) => {
        try {
            ...
            if (authHeader && authHeader.startsWith('Bearer ')) {
                try {
                    const userDecoded = jwt.verify(authHeader.split(' ')[1], JWT_SECRET);
    ...
    router.get('/:meetingId/report', protect, async (req, res) => {
        try {
            ...
            const attendedIds = new Set(attendance.map(a => a.userId._id.toString()));
            const invitees = meeting.participants.map(p => ({ _id: p._id, name: p.name, email: p.email }));
            const attended = attendance.map(a => ({
                user: a.userId, method: a.method,
                joinTimestamp: a.joinTimestamp, punctual: a.punctual,
            }));
            const absent = invitees.filter(p => !attendedIds.has(p._id.toString()));
...
function attendancePage(message, success) {
