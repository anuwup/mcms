...
async function generateBrief(meeting, callAISummarize) {
    ...
    const context = {
        ...
        agendaItems: agendaItems.map(i => ({ title: i.title, duration: i.duration })),
    ...
    const agendaList = agendaItems.length > 0
        ? agendaItems.map((item, idx) => `${idx + 1}. ${item.title} (${item.duration} min)`).join('\n')
        : 'No agenda items set yet.';

    const actionList = pendingActions.length > 0
        ? pendingActions.map(a => `- ${a.title} (${a.assigneeName || a.assignee?.name || 'Unassigned'}) — ${a.status}`).join('\n')
...
function formatBriefEmail(brief, meetingId, clientUrl) {
...
module.exports = { generateBrief, formatBriefEmail };
